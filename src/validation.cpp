                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    bool fScriptChecks = !::ChainstateActive().IsInitialBlockDownload();
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        else
        {
            ...
            if (!Consensus::CheckTxInputs(tx, tx_state, view, pindex->nHeight, txfee, (pindex->pprev? pindex->pprev->nMoneySupply : 0))) {
    ...
    else if (block.IsProofOfStake())
    {
        ...
        if (block.nVersion <= 4) {
            ...
            double fInflationAdjustment = GetInflationAdjustment(pindex->pprev, chainparams.GetConsensus());
...
CBlockIndex* BlockManager::AddToBlockIndex(const CBlockHeader& block)
{
    ...
    if (!pindexNew->SetStakeEntropyBit(StakeEntropyBitFromHash(hash))) {
...
void CChainState::ReceivedBlockTransactions(const CBlock& block, CBlockIndex* pindexNew, const FlatFilePos& pos, const Consensus::Params& consensusParams)
{
    ...
    if (!VerifyHashTarget(pindexNew->pprev, block, hash)) {
        return;
    }
}
...
static bool ContextualCheckBlock(const CBlock& block, BlockValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // coinbase height is wrong past this height..
    if (nHeight < consensusParams.nRevertCoinbase && pindexPrev != nullptr)
    {
...
bool CChainState::AcceptBlock(const std::shared_ptr<const CBlock>& pblock, BlockValidationState& state, const CChainParams& chainparams, CBlockIndex** ppindex, bool fRequested, const FlatFilePos* dbp, bool* fNewBlock)
{
    ...
    //! dont bother checking nBits for PoW
    if (block.IsProofOfStake() && (block.nBits != GetNextWorkRequired(pindex->pprev, &block, chainparams.GetConsensus())))
...
double GetInflationAdjustment(const CBlockIndex* pindex, const Consensus::Params& consensusParams)
{
    ...
    if (g_chainman.m_blockman.m_block_index.count(hash) == 0)
        LogPrintf("- Hash block missing\n");

    int64_t nMoneySupplyPrev = g_chainman.m_blockman.m_block_index[hash]->nMoneySupply;
    int64_t nHeightPrev = g_chainman.m_blockman.m_block_index[hash]->nHeight;
